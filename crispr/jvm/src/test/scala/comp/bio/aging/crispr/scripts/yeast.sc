...
object Cells {
  ...
  val yeast = Assembly("Saccharomyces cerevisiae", "${root}/genomes/YEAST/S288C_reference_genome_R64-2-1_20150113") //toDO: add others

  /* ... new cell ... */

  val current = yeast
  val gff = sparkContext.loadGff3(s"${current.path}/saccharomyces_cerevisiae_R64-2-1_20150113.gff")
  gff.saveAsParquet(current.featuresAdam)
  ...
  gff.rdd.takeSample(true, 10).toList
  /* ... new cell ... */
  ...
  val transcriptFeatures = features.toDF.where($"featureType" === "transcript")

  val transcripts = lagsWithId
    .join(transcriptFeatures, $"gene_id" === $"geneId")
  ...
  (selection.count, lagsWithId.count, transcripts.count, transcriptFeatures.count)
